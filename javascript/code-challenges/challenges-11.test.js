...
const count = (target, input) => {
  let counter = 0;
  input.map((target) => ( target === target && counter++));
  return counter;
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(row => {
    row.filter(cell => typeof cell === 'number' && % 5 === 0).map( cell => Math.pow(2, cell));

    });
  }
  // Solution code here... Solved in Class
};
